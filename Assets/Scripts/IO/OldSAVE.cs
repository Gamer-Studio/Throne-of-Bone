...
namespace ToB.IO
{
  ...
  /// </summary>
  [Serializable, Obsolete("새로운 형태로 구현중에 있습니다. 사용하지 말아주세요!")]
  public class OldSAVE : JObject
  {
    ...
    public OldSAVE(string name, bool isRoot = true)
    {
      this.name = name;
    ...
    public static OldSAVE Current = null;
    ...
    public static async Task<OldSAVE> Load(string name, bool create = false)
    {
      var path = System.IO.Path.Combine(SavePath, name + ".json");
    ...
    public static OldSAVE LoadSync(string name, bool create = false)
    {
    ...
    /// </summary>
    /// <param name="name">로드할 저장 파일의 이름</param>
    /// <param name="oldSave">로드된 SAVE 객체</param>
    /// <returns>로드 성공 여부</returns>
    public static bool TryGet(string name, out OldSAVE oldSave)
    {
      oldSave = null;
      if (Exist(name))
      {
