    ...
    public void Attack(Vector2 direction, bool isMelee = true, bool bottomAttack = false)
    {
      ...
      if (isMelee)
      {
        ...
        if(bottomAttack && IsFlight)
        {
          ...
          if (!isDamageImmune) AudioManager.Play("fntgm_blade_whoosh_light_02", AudioType.Effect); // 아래공격 사운드
        }
        else
        {
          ...
          if (prevAttackMotion == 0 && !isDamageImmune) AudioManager.Play("fntgm_blade_whoosh_light_02",AudioType.Effect); // 1타
          else if (prevAttackMotion == 1 && !isDamageImmune) AudioManager.Play("fntgm_blade_whoosh_med_03",AudioType.Effect); // 2타
          else if (prevAttackMotion == 2 && !isDamageImmune) AudioManager.Play("fntgm_blade_whoosh_heavy_03",AudioType.Effect); // 3타
